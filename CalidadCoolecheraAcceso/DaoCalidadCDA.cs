...
namespace CalidadCoolecheraAcceso
{
    public class DaoCalidadCDA
    {
        ...
        {
            var Lsql = "SELECT * FROM calidad_cda";
        ...
        {
            var strSQl = "select * from calidad_cda where cd_codigocda = @cd_codigocda";
        ...
        {
            var strSQl = @"INSERT INTO calidad_cda(cd_codigocda, ds_periodoliquidacion, am_numeroliquidacion, am_valorsolidostotales, am_valorunidadformadoracolonias, ds_usuariocreacion, ds_equipocreacion, dt_fechacreacion, ds_programacreacion) 
                           VALUES (@cd_codigocda,@ds_periodoliquidacion, @am_numeroliquidacion, @am_valorsolidostotales, @am_valorunidadformadoracolonias, @ds_usuariocreacion, @ds_equipocreacion, @dt_fechacreacion, @ds_programacreacion)  ";
        ...
        {
            var strSQl = @"UPDATE calidad_cda SET ds_periodoliquidacion = @ds_periodoliquidacion, am_numeroliquidacion =@am_numeroliquidacion, am_valorsolidostotales = @am_valorsolidostotales, am_valorunidadformadoracolonias  = @am_valorunidadformadoracolonias,ds_usuariomodificacion= @ds_usuariomodificacion, ds_equipomodificacion = @ds_equipomodificacion, dt_fechamodificacion = @dt_fechamodificacion, ds_programamodificacion = @ds_programamodificacion
        ...
        {
            var strSQl = @"DELETE FROM calidad_cda WHERE cd_codigocda= @cd_codigocda";
            var nroFilasAfectadas = pConexion.Conexion.Execute(strSQl, CalidadCDA, transaction);
            return nroFilasAfectadas;
        }
    }
