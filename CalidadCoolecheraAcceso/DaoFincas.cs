...
namespace CalidadCoolecheraAcceso
{
    public class DaoFincas
    {
        ...
        {
            var strSQl = "SELECT f.codfinca, f.nombre, f.codruta, p.nombre as codplanta, f.codigosap  FROM calidad.mtfinca f JOIN calidad.mtplanta p ON p.codplanta = f.codplanta where codconsignante = @codconsignante AND codigosap IS NOT NULL";
